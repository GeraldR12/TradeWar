...
/**
 * Turns raw TradeDataManager state into per-town TradeStatus objects.
 * No Bukkit/BlueMap/Towny types here on purpose: TradeDataManager stays the only
 * source of truth, this class just resolves names against the live Town/Nation
 * universe and merges tariffs/sanctions/embargoes into one status per town.
 * Unresolvable or expired policies are dropped (never surfaced as a marker) and
 * reported through the warn sink instead of throwing.
 */
        ...
        for (Map.Entry<String, List<TradeDataManager.TariffRule>> entry : tariffRules.entrySet()) {
            ...
            for (TradeDataManager.TariffRule rule : entry.getValue()) {
                if (rule.expiryTime() > 0 && now > rule.expiryTime()) {
                    continue; // expired tariffs are never shown, not an error
                }
