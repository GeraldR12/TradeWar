...
/**
 * Optional bridge between TradeWar and BlueMap. Only ever constructed after confirming
 * the BlueMap plugin is present, so this is the only class besides TradeMarkerManager
 * that references BlueMapAPI classes - if that guard ever changes, keep it that way,
 * otherwise a server without BlueMap installed would fail to load those classes.
 */
